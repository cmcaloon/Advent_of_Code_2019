...
public class WireMapCross {
    ...
    static void outputGrid(HashMap<Integer, HashMap<Integer, String>> wireMap) {
        ...
        for (int x = 0; x < directionSize; x++){
            ...
            char movement = wireDirections1[x].charAt(0);
            String toInt = wireDirections1[x];
            ...
            switch(movement)
            {
                case 'U':
                    for(int y = 0; y < moveVal; y++) {
                        posY++;
                        if (wireMap.containsKey(posX) == true && wireMap.get(posX).containsKey(posY) == true){
                            if (wireMap.get(posX).get(posY) == "-") {
                                columnList.put(posY, "x");
                            } else
                    ...
                    //System.out.println("Adding UP "+ wireMap.get(posX).get(posY) +" to "+ posX +":"+posY);
                    break;
                case 'D':
                    for(int y = 0; y < moveVal; y++) {
                        posY--;
                        if (wireMap.containsKey(posX) == true && wireMap.get(posX).containsKey(posY) == true) {
                            if (wireMap.get(posX).get(posY) == "-") {
                                ...
                                System.out.println("Unexpected Char Down " + wireMap.get(posX).get(posY));
                        ...
                        wireMap.put(posX, columnList);
                        System.out.println("Adding DOWN " + wireMap.get(posX).get(posY) + " to " + posX + ":" + posY);
                    }
                    break;
                case 'L':
                    for(int y = 0; y < moveVal; y++) {
                        ...
                        if (wireMap.containsKey(posX) == true && wireMap.get(posX).containsKey(posY) == true){
                            if (wireMap.get(posX).get(posY) == "-") {
                                ...
                                String current = wireMap.get(191919).get(135135);
                                columnList.put(posY, "?");
                                wireMap.put(posX, columnList);
                            }
                        }
                        ...
                        System.out.println("Adding LEFT "+ wireMap.get(posX).get(posY) +" to "+ posX +":"+posY);
                    }
                    break;
                case 'R':
                    for(int y = 0; y < moveVal; y++) {
                        posX++;
                        //String current = wireMap.get(posX).get(posY);
                        if (wireMap.containsKey(posX) == true && wireMap.get(posX).containsKey(posY) == true){
                            if (wireMap.get(posX).get(posY) == "-") {
                                columnList.put(posY, "x");
                            } else {
                        ...
                        wireMap.put(posX, columnList);
                        //System.out.println("Adding RIGHT "+ wireMap.get(posX).get(posY) +" to "+ posX +":"+posY);
                    }
                    break;
                default:
        ...
        /*for (int x = 0; x < 2000; x++)
        {
            for(int y = 0; y < 2000; y++){
                System.out.printf(wireMap.get(x).get(y));
            }
            System.out.println("");
        }
        */
    ...
    public static void generateGrid() {
        HashMap<Integer, HashMap<Integer, String>> wireMap = new HashMap<Integer, HashMap<Integer, String>>();
